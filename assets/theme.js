            ...
            if (main_slider) {
                ...
                this.flickityInstance.on('select', function (index) {
                    ...
                    if (selectedSlideModel) {
                        ...
                        if (modelViewerEl) {
                            modelViewerEl.dispatchEvent(new CustomEvent('mediaVisible', {
                                bubbles: true,
                                cancelable: true
                            }));
                        }
                        ...
                        if (modelViewerSlides.length > 0) {
                            modelViewerSlides.forEach(function (item, index) {
                                ...
                                if (modelViewerEl) {
                                    modelViewerEl.dispatchEvent(new CustomEvent('mediaHidden', {
                                        bubbles: true,
                                        cancelable: true
                                    }));
                                }
    ...
    function init() {
        ...
        if (search_submit_btn) {
            search_submit_btn.addEventListener('click', function (e) {
                ...
                var productQuery = '' + formValue + '*' + (formProductType !== '' ? '&' + 'product_type:' + formProductType : '');
        ...
        if (qty_btns.length) {
            qty_btns.forEach(function (item, index) {
                item.addEventListener('click', function (e) {
                    ...
                    const qtyParent = item.closest('.quantity_block');
                    const productErrorBlock = qtyParent.nextElementSibling;
                    var inputEl = this.closest('.qty').querySelector('[name="quantity"]');
                    var qty = +(inputEl.value);

                    if (this.classList.contains('minus_btn')) {
                        productErrorBlock.classList.add('product-form__error-message-wrapper--hidden');
                        qty--;
                    } else {
                        if (item.getAttribute('data-inventory-quantity') && qty >= item.getAttribute('data-inventory-quantity')) {
                            productErrorBlock.classList.remove('product-form__error-message-wrapper--hidden');
                            return;
                        }
                        else {
                            productErrorBlock.classList.add('product-form__error-message-wrapper--hidden');
